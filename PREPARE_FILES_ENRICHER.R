...
# CORE_GENES <- read.table(paste0(DEG_dir,"/","LFC_CORE.tsv"),header = T)
# PAN_GENES <- read.table(paste0(DEG_dir,"/","LFC_PAN.tsv"),header = T)
#alternative
#define categories joining MC and HC together for up and down regulated genes
#core
################################################################################
#to join HC and MC to enlarge dataset for motif enrichment
CORE_GENES$RES_HC2 <- NA
CORE_GENES$RES_HC2[CORE_GENES$RES_HC %in% c("UP_HC","UP_MC")] <- "UP"
CORE_GENES$RES_HC2[CORE_GENES$RES_HC %in% c("DOWN_HC","DOWN_MC")] <- "DOWN"
#pan
PAN_GENES$RES_HC2 <- NA
PAN_GENES$RES_HC2[PAN_GENES$RES_HC %in% c("UP_HC","UP_MC")] <- "UP"
PAN_GENES$RES_HC2[PAN_GENES$RES_HC %in% c("DOWN_HC","DOWN_MC")] <- "DOWN"

################################################################################
#printing new numbers for pan
tapply(PAN_GENES$RES_HC2,PAN_GENES$RES_HC2,length)
#printing new numbers for core
tapply(CORE_GENES$RES_HC2,CORE_GENES$RES_HC2,length)
################################################################################
###splitting files per categories
##apply for normal groups (silenced)
# cats <- unique(PAN_GENES$RES_HC) 
# cats <- cats[c(4,5,6,7)]
cats <- unique(PAN_GENES$RES_HC2) 
cats <- na.omit(cats)#cats[c(2,3)]
###############################################################################
...
for(i in seq_len(length(cats))){
 ...
 write.table(row.names(CORE_GENES[which(CORE_GENES$RES_HC2==cats[i]),]),
             paste0(in_dir,"/","CORE_",cats[[i]],".tsv"),sep = "\t",na = "",quote = F,col.names = F,row.names = F)
 write.table(row.names(PAN_GENES[which(PAN_GENES$RES_HC2==cats[i]),]),
...
#saving the bash file
write.table(unlist(final_list),
            paste0(xx_dir,"/","MOTIF_ENRICHMENT_RES.sh"),sep = "\t",na = "",quote = F,col.names = F,row.names = F)
